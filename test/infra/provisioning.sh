...
if [[ -z $(which ansible-playbook) ]]; then
    ...
    #upgrade ansible 2.7 to latest version by pip (don't do like this on production servers)))
    python -m pip install --upgrade ansible    
fi

# vagrant box provision
env PYTHONUNBUFFERED=1 ANSIBLE_HOST_KEY_CHECKING=0 ANSIBLE_FORCE_COLOR=1 ANSIBLE_ROLES_PATH=${PROJECT_ROOT}/test/infra/roles \
        ansible-playbook \
        "${PROJECT_ROOT}/test/infra/vagrant.yml"
